...
export class DataManager {
  ...
  async saveAverageResaleProfit(profit: number): Promise<boolean> {
    try {
      ...
      if (error) {
        console.error('❌ [DataManager] Erro ao salvar lucro médio de produtos de revenda no Supabase:', error);
        return false;
      }
  ...
  /**
   * Salva as configurações de lucro empresarial
   */
  async saveEmpresarialProfitSettings(settings: any): Promise<boolean> {
    try {
      console.log('💾 [DataManager] Salvando configurações de lucro empresarial...', settings);

      const { error } = await this.supabase
        .from('system_settings')
        .upsert({
          key: 'empresarial_profit_settings',
          value: JSON.stringify(settings),
          updated_at: new Date().toISOString(),
          description: 'Configurações do sistema de lucro empresarial'
        }, {
          onConflict: 'key'
        });

      if (error) {
        console.error('❌ [DataManager] Erro ao salvar configurações no Supabase:', error);
        return false;
      }

      console.log('✅ [DataManager] Configurações de lucro empresarial salvas com sucesso');
      return true;

    } catch (error) {
      console.error('❌ [DataManager] Erro crítico ao salvar configurações:', error);
      return false;
    }
  }

  /**
   * Carrega as configurações de lucro empresarial
   */
  async loadEmpresarialProfitSettings(): Promise<any | null> {
    try {
      console.log('📖 [DataManager] Carregando configurações de lucro empresarial...');

      const { data, error } = await this.supabase
        .from('system_settings')
        .select('*')
        .eq('key', 'empresarial_profit_settings')
        .single();

      if (error) {
        if (error.code === 'PGRST116') {
          console.log('ℹ️ [DataManager] Configurações não encontradas, usando padrões');
          return null;
        }
        throw error;
      }
      ...
      if (data && data.value) {
        ...
        console.log('✅ [DataManager] Configurações carregadas com sucesso:', settings);
        return settings;
      }
      ...
      console.error('❌ [DataManager] Erro ao carregar configurações:', error);
      return null;
    }
  }

  /**
   * Subscription para mudanças nas configurações de lucro empresarial
   */
  subscribeToEmpresarialProfitSettingsChanges(callback: (settings: any) => void): () => void {
    console.log('🔔 [DataManager] Configurando subscription para configurações de lucro empresarial...');

    const channel = this.supabase
      .channel('empresarial_profit_settings')
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'system_settings',
          filter: 'key=eq.empresarial_profit_settings'
        },
        (payload) => {
          console.log('📡 [DataManager] Mudança detectada nas configurações:', payload);
          
          if (payload.new && payload.new.value) {
            try {
              const settings = JSON.parse(payload.new.value);
              callback(settings);
            } catch (error) {
              console.error('❌ [DataManager] Erro ao parsear configurações:', error);
            }
          }
        }
      )
    ...
    return () => {
      console.log('🔕 [DataManager] Cancelando subscription das configurações');
      channel.unsubscribe();
    };
  }taManager] Erro ao salvar no Supabase:', error);
        return false;
      }
