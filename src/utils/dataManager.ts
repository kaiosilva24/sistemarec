...
export class DataManager {
  ...
  async saveCashBalance(balance: number): Promise<boolean> {
    try {
      ...
      console.log('✅ [DataManager] Saldo de caixa salvo no Supabase com sucesso');
      return true;
    } catch (error) {
      console.error('❌ [DataManager] Erro crítico ao salvar saldo de caixa:', error);
      return false;
    }
  }
  ...
  async loadCashBalance(): Promise<number> {
    try {
      ...
      if (error || !data) {
        console.warn('⚠️ [DataManager] Saldo de caixa não encontrado no Supabase, calculando...');
        
        // Se não existe, calcular baseado nas entradas de cash flow
        ...
        await this.saveCashBalance(balance);
        return balance;
      }
  ...
  /**
   * Subscreve às mudanças do saldo de caixa em tempo real
   */
  subscribeToCashBalanceChanges(callback: (balance: number) => void): () => void {
    console.log('🔔 [DataManager] Iniciando subscription para saldo de caixa...');
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.new && typeof payload.new === 'object' && 'value' in payload.new) {
            ...
            console.log(`💰 [DataManager] Novo saldo de caixa recebido: R$ ${newBalance.toFixed(2)}`);
