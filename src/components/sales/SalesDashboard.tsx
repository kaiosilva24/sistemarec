...
import {
  ...
  useMaterials,
  useResaleProducts,
  useInstallmentSales, // Import the hook for installment sales
...
import {
  ...
  StockItem,
  ResaleProduct,
  InstallmentSale, // Import the type for installment sales
...
const SalesDashboard = ({
  ...
  const [productType, setProductType] = useState<
    "final" | "resale" | "warranty" | "installment" // Added "installment"
  >("final");
  ...
  // Installment Sale specific states
  ...
  const { installmentSales, addInstallmentSale, updateInstallmentSale } = useInstallmentSales(); // Use the hook for installment sales
  ...
  useEffect(() => {
    if (
      ...
      productType !== "installment" && // Don't calculate for installments here
      unitPrice &&
      quantity &&
  ...
  const handleConfirmSale = async () => {
    ...
    if (productType === "warranty") {
      if (
       ...
       if (
        ...
        parseInt(numberOfInstallments) <= 0 ||
        !firstInstallmentDate ||
        !paymentMethod // Assuming payment method is also required for installments
      ) {
        alert("Por favor, preencha todos os campos corretamente para parcelamento.");
        return;
      }
    }
    else {
    ...
    try {
      if (productType === "warranty") {
        ...
        // Handle installment sale
        if (!product && !resaleProduct) {
          alert("Erro: Produto não encontrado para parcelamento.");
          return;
        }

        const productName = product
          ? product.item_name
          : resaleProduct?.name || "Produto Desconhecido";
        const productUnit = product?.unit || resaleProduct?.unit || "";
        const productId = product?.id || resaleProduct?.id;

        const installmentSaleData: InstallmentSale = {
          id: "", // Auto-generated by backend/database
          customer_id: customer.id,
          customer_name: customer.name,
          ...
          salesperson_name: salesperson.name,
          total_value: parseFloat(installmentValue),
        ...
        // Add installment sale record
        await addInstallmentSale(installmentSaleData);
        ...
        alert(
          `Venda parcelada registrada com sucesso!\n\n` +
          `Cliente: ${customer.name}\n` +
          `Produto: ${productName}\n` +
          `Valor Total: ${formatCurrency(parseFloat(installmentValue))}\n` +
          `Parcelas: ${numberOfInstallments}\n` +
          `Primeira Parcela: ${new Date(firstInstallmentDate).toLocaleDateString('pt-BR')}\n` +
          `Vendedor: ${salesperson.name}\n\n` +
          `📦 Estoque atualizado automaticamente`,
        );
      ...
      console.error(
        `Erro ao registrar ${productType === "warranty" ? "garantia" : productType === "installment" ? "venda parcelada" : "venda"}:`,
        error,
      );
      alert(
        `Erro ao registrar ${productType === "warranty" ? "a garantia" : productType === "installment" ? "a venda parcelada" : "a venda"}. Tente novamente.`,
      );
    } finally {
  ...
  const handleProductSelect = (product: any) => {
    ...
    // If it's a warranty sale, set quantity directly if it's the only product
    if (productType === "warranty" && !quantity) {
      setQuantity("1"); // Default to 1 for warranty
    }

    // For installment sales, set default installment value based on product price
    if (productType === "installment") {
      if (productType === "final") {
        setUnitPrice(product.unit_cost.toString()); // Set unit price for calculation
      } else { // Resale product
        const stockItem = stockItems.find(
          (item) => item.item_id === product.id && item.item_type === "product",
        );
        setUnitPrice(stockItem?.unit_cost?.toString() || "0");
      }
    }
  };
        ...
        <TabsContent value="pos">
          <Card className="bg-factory-900/80 border-tire-600/30 max-w-7xl mx-auto">
            ...
            <CardContent className="space-y-6 bg-factory-900/60 rounded-lg">
              ...
              {selectedProduct && productType !== "warranty" && productType !== "installment" && (
              ...
              {/* Step 4 Alt: Installment Value (only for installments) */}
              {selectedProduct && productType === "installment" && (
                <div className="space-y-2">
                  <Label
                    htmlFor="installmentValue"
                    className="text-tire-300 font-medium"
                  >
                    4. Valor Total da Venda (R$) *
                  </Label>
                  <Input
                    id="installmentValue"
                    type="number"
                    ...
                    onChange={(e) => {
                      ...
                      // Update saleValue for potential display or further calculations
                      setSaleValue(e.target.value);
                    }}
                    className="bg-factory-700/50 border-tire-600/30 text-white h-12 text-lg"
                    placeholder="Valor total a ser parcelado"
                  />
                  <p className="text-tire-400 text-sm">
                    💰 Informe o valor total da venda a ser parcelada
                  </p>
                </div>
              )}
              ...
              {selectedProduct && (
                <div className="space-y-2">
                  <Label
                    ...
                    className="text-tire-300 font-medium"
                  >
                    {productType === "warranty" ? "4" : productType === "installment" ? "5" : "5"}. Quantidade *
                  </Label>
                  <Input
                    id="quantity"
                    type="number"
              ...
              {/* Step 6: Installment Details (only for installments) */}
              {selectedProduct && productType === "installment" && (
                <>
                  <div className="space-y-2">
                    <Label
                ...
                quantity && (
                  <div className="space-y-2">
                    <Label
                      ...
                      className="text-tire-300 font-medium"
                    >
                      {productType === "warranty" ? "4" : productType === "installment" ? "8" : "6"}. Valor Total da Venda (R$) *
                    </Label>
                    <div className="relative">
                      <Input
                        id="saleValue"
              ...
              {selectedProduct && productType === "installment" && (!installmentValue || !numberOfInstallments || !firstInstallmentDate) && (
                <div className="p-3 bg-blue-700/20 rounded border border-blue-600/30">
                  <p className="text-blue-400 text-sm text-center">
                    ⏳ Preencha o valor total, número de parcelas e data da primeira parcela.
                  </p>
                </div>
              )}
              ...
              {selectedProduct && productType !== "warranty" && (
                <div className="space-y-2">
                  <Label className="text-tire-300 font-medium">
                    {productType === "installment" ? "8" : productType === "warranty" ? "4" : "7"}. Forma de Pagamento *
                  </Label>
                  <Select
                    value={paymentMethod}
              ...
              {productType !== "warranty" && productType !== "installment" &&
                selectedSalesperson &&
                selectedCustomer &&
                selectedProduct &&
                unitPrice &&
                ...
                paymentMethod && (
                  <div className="p-4 bg-blue-500/10 rounded-lg border border-blue-500/30">
                    <h4 className="text-blue-400 font-medium mb-3 flex items-center gap-2">
                      <span className="text-lg">📅</span> Resumo da Venda Parcelada:
                    </h4>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-tire-300">Vendedor:</span>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-blue-400 font-bold text-lg">
                          {formatCurrency(parseFloat(installmentValue))}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-tire-300">Parcelas:</span>
                        <span className="text-blue-400 font-bold">
                          {numberOfInstallments}x
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-tire-300">Primeira Parcela:</span>
                        <span className="text-blue-400 font-bold">
              ...
              <Button
                ...
                disabled={
                  ...
                  (productType !== "warranty" && productType !== "installment" &&
                    (!unitPrice ||
                      ...
                      parseFloat(unitPrice) <= 0 ||
                      parseFloat(saleValue) <= 0 ||
                      !paymentMethod)) || // Check paymentMethod for regular sales
                  (productType === "installment" && (!installmentValue || parseFloat(installmentValue) <= 0 || !numberOfInstallments || parseInt(numberOfInstallments) <= 0 || !firstInstallmentDate || !paymentMethod)) // Check all fields for installments
                }
                className={`w-full h-14 text-lg font-medium text-white ${
                  productType === "warranty"
                    ...
                    : productType === "installment"
                    ? "bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-700 hover:to-blue-500"
              ...
              <div className="text-center text-tire-400 text-sm">
                {productType === "warranty" ? (
                  ...
                  <>
                    <p>📅 Preencha os campos para registrar a venda parcelada.</p>
                    <p className="mt-1">
                      O valor total será dividido pelo número de parcelas.
                    </p>
                  </>
                ) : (
                  <>
                  ...
                  finalProductSalesHistory.map((sale) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center gap-2">
                            {/* Check if this is an installment sale that was received */}
                            {sale.description && 
                             sale.description.includes("Pagamento: À Prazo") && 
                             !sale.description.includes("STATUS: Recebido") && (
                              <Button
                                variant="outline"
                                size="sm"
                                className="text-green-400 border-green-400 hover:bg-green-500/10"
                                onClick={() => { /* TODO: Implement handleReceiveInstallment */ alert("Funcionalidade de recebimento de parcelas ainda não implementada."); }}
                              >
                                Receber
                              </Button>
                            )}
