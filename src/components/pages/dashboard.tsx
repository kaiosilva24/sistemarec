...
const MainDashboard = ({ isLoading = false }: { isLoading?: boolean }) => {
  ...
  // Estados para sincronização via arquivo
  ...
  // SISTEMA DE SINCRONIZAÇÃO VIA ARQUIVO JSON
  const exportMetricsToFile = () => {
    try {
      console.log("📤 [Dashboard] INICIANDO EXPORTAÇÃO para arquivo...");
      
      // Coletar TODOS os dados de métricas e configurações
      const exportData = {
        // Dados das métricas calculadas
        metrics: {
          averageCostPerTire,
        ...
        tireCostManagerSync: JSON.parse(
          localStorage.getItem("tireCostManager_synchronizedCostData") || "{}"
        ),
        
        // Dados de persistência individuais
        persistence: {
          averageCostPerTire: JSON.parse(
            localStorage.getItem("dashboard_averageCostPerTire") || "{}"
          ),
          averageProfitPerTire: JSON.parse(
            localStorage.getItem("dashboard_averageProfitPerTire") || "{}"
          ),
          profitPercentage: JSON.parse(
            localStorage.getItem("dashboard_profitPercentage") || "{}"
          ),
        },
        ...
        metadata: {
          ...
          exportedBy: "Dashboard Metrics System",
          description: "Backup completo das métricas e configurações do dashboard",
          browserInfo: {
      ...
      const now = new Date();
      const fileName = `dashboard-metrics-${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}-${String(now.getMinutes()).padStart(2, '0')}.json`;
      ...
      console.log("✅ [Dashboard] EXPORTAÇÃO CONCLUÍDA:", {
        fileName,
      ...
      // Notificar sucesso
      alert(`✅ Arquivo de métricas exportado com sucesso!\n\nArquivo: ${fileName}\nTamanho: ${(dataStr.length / 1024).toFixed(2)} KB`);
      
    } catch (error) {
      console.error("❌ [Dashboard] ERRO na exportação:", error);
      alert("❌ Erro ao exportar métricas. Verifique o console para detalhes.");
    }
  };

  const importMetricsFromFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;

    console.log("📥 [Dashboard] INICIANDO IMPORTAÇÃO do arquivo:", file.name);
    ...
    reader.onload = (e) => {
      try {
        ...
        console.log("📊 [Dashboard] DADOS IMPORTADOS:", importData);
        
        // Validar estrutura do arquivo
        if (!importData.metadata || !importData.metrics) {
          throw new Error("Arquivo de métricas inválido - estrutura incorreta");
        }
        
        console.log("🔄 [Dashboard] APLICANDO SINCRONIZAÇÃO COMPLETA...");
        
        // 1. Sincronizar métricas principais
        if (importData.metrics.averageCostPerTire) {
          setAverageCostPerTire(importData.metrics.averageCostPerTire);
          localStorage.setItem("dashboard_averageCostPerTire", JSON.stringify({
            value: importData.metrics.averageCostPerTire,
            timestamp: Date.now(),
            source: "File_Import_Sync"
          }));
        }
        
        if (importData.metrics.averageProfitPerTire) {
          setAverageProfitPerTire(importData.metrics.averageProfitPerTire);
          localStorage.setItem("dashboard_averageProfitPerTire", JSON.stringify({
            value: importData.metrics.averageProfitPerTire,
            timestamp: Date.now(),
            source: "File_Import_Sync"
          }));
        }
        
        if (importData.metrics.profitPercentage) {
          setProfitPercentage(importData.metrics.profitPercentage);
          localStorage.setItem("dashboard_profitPercentage", JSON.stringify({
            value: importData.metrics.profitPercentage,
            timestamp: Date.now(),
            source: "File_Import_Sync"
          }));
        }
        
        // 2. Sincronizar configurações do dashboard
        if (importData.dashboard) {
          if (importData.dashboard.cardOrder) {
            saveCardOrder(importData.dashboard.cardOrder);
          }
          if (importData.dashboard.customColors) {
            saveCustomColors(importData.dashboard.customColors);
          }
          if (importData.dashboard.hiddenCards) {
            saveHiddenCards(importData.dashboard.hiddenCards);
          }
        }
        
        // 3. Sincronizar dados do TireCostManager
        if (importData.tireCostManagerSync && Object.keys(importData.tireCostManagerSync).length > 0) {
          localStorage.setItem(
            "tireCostManager_synchronizedCostData",
            JSON.stringify({
              ...importData.tireCostManagerSync,
              lastSyncedAt: new Date().toISOString(),
              syncSource: "File_Import"
            })
          );
        }
        ...
        const importSummary = {
          arquivo: file.name,
          dataExportacao: importData.metadata.exportedAt,
          versao: importData.metadata.version,
          custoPorPneu: importData.metrics.averageCostPerTire || "N/A",
          lucroPorPneu: importData.metrics.averageProfitPerTire || "N/A",
          porcentagemLucro: importData.metrics.profitPercentage || "N/A",
          configuracoesDashboard: !!importData.dashboard,
          dadosTireCostManager: !!importData.tireCostManagerSync
        };
        
        console.log("🎉 [Dashboard] SINCRONIZAÇÃO COMPLETA:", importSummary);
        ...
        setTimeout(() => {
          ...
          alert(`🎉 SINCRONIZAÇÃO CONCLUÍDA COM SUCESSO!\n\n` +
            `📁 Arquivo: ${file.name}\n` +
            `📅 Exportado em: ${new Date(importData.metadata.exportedAt).toLocaleString('pt-BR')}\n` +
            `💰 Custo por Pneu: R$ ${importData.metrics.averageCostPerTire?.toFixed(2) || 'N/A'}\n` +
            `💎 Lucro por Pneu: R$ ${importData.metrics.averageProfitPerTire?.toFixed(2) || 'N/A'}\n` +
            `📊 Lucro %: ${importData.metrics.profitPercentage?.toFixed(1) || 'N/A'}%\n\n` +
            `✅ Todas as métricas foram sincronizadas!`);
        }, 1000);
        
      } catch (error) {
        console.error("❌ [Dashboard] ERRO na importação:", error);
        setIsSyncing(false);
        alert(`❌ Erro ao importar métricas:\n\n${error instanceof Error ? error.message : 'Erro desconhecido'}\n\nVerifique se o arquivo é válido.`);
      }
    };
  ...
  const syncWithBrowser = () => {
    try {
      console.log("🔄 [Dashboard] SINCRONIZAÇÃO MANUAL ATIVADA");
      
      // Forçar releitura de todos os dados
      const readTireCostManagerValue = () => {
        try {
          // Método 1: Leitura do localStorage atualizado
          ...
          if (unifiedData) {
            ...
            if (parsed.averageCostPerTire && parsed.averageCostPerTire > 0) {
              console.log(`✅ [Dashboard] SINCRONIZAÇÃO MANUAL - Custo: R$ ${parsed.averageCostPerTire.toFixed(2)}`);
              setAverageCostPerTire(parsed.averageCostPerTire);
              return true;
            }
          }
          
          // Método 2: Leitura do localStorage específico
          const savedData = localStorage.getItem("dashboard_averageCostPerTire");
          if (savedData) {
            const parsed = JSON.parse(savedData);
            if (parsed.value && parsed.value > 0) {
              console.log(`✅ [Dashboard] SINCRONIZAÇÃO MANUAL - Custo Salvo: R$ ${parsed.value.toFixed(2)}`);
      ...
      const success = readTireCostManagerValue();
      
      if (success) {
        alert("✅ SINCRONIZAÇÃO MANUAL CONCLUÍDA!\n\nDados atualizados com sucesso.");
      } else {
        alert("⚠️ Nenhum dado encontrado para sincronizar.\n\nVerifique se há dados salvos no TireCostManager.");
      }
      
    } catch (error) {
      console.error("❌ [Dashboard] Erro na sincronização manual:", error);
      alert("❌ Erro na sincronização manual. Verifique o console.");
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            onClick={syncWithBrowser}
            className="bg-neon-green hover:bg-neon-green/80 text-white flex items-center gap-2"
          >
            <RefreshCw className="h-4 w-4" />
            Sincronizar
          </Button>

          {/* Botão de Backup/Restore */}
          <Dialog open={showSyncDialog} onOpenChange={setShowSyncDialog}>
            <DialogTrigger asChild>
              <Button
                variant="outline"
                className="bg-neon-blue/20 border-neon-blue/50 text-neon-blue hover:bg-neon-blue/30 flex items-center gap-2"
              >
            ...
            <DialogContent className="bg-factory-800 border-tire-600/30 text-white max-w-2xl">
              <DialogHeader>
                <DialogTitle className="text-white flex items-center gap-2">
                  ...
                  Sistema de Sincronização via Arquivo
                </DialogTitle>
                <DialogDescription className="text-tire-300">
                  Exporte suas métricas para usar em outros navegadores ou faça backup dos dados
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-6 py-4">
                ...
                <div className="space-y-3">
                  <h4 className="text-white font-medium flex items-center gap-2">
                    <span className="text-lg">📤</span>
                    Exportar Métricas (Backup)
                  </h4>
                  <p className="text-tire-400 text-sm">
                    Gera um arquivo JSON com todas as métricas e configurações atuais
                  </p>
                  <Button
                    onClick={exportMetricsToFile}
                    className="bg-neon-green hover:bg-neon-green/80 text-white flex items-center gap-2"
                  >
                    <span className="text-lg">💾</span>
                    Baixar Arquivo de Métricas
                  </Button>
                </div>

                {/* Separador */}
                <div className="border-t border-tire-600/30"></div>

                {/* Importar Dados */}
                <div className="space-y-3">
                  <h4 className="text-white font-medium flex items-center gap-2">
                    <span className="text-lg">📥</span>
                    Importar Métricas (Restaurar)
                  </h4>
                  <p className="text-tire-400 text-sm">
                    Carrega métricas de um arquivo JSON para sincronizar com outro navegador
                  </p>
                  <div className="flex items-center gap-3">
                    <Input
                      type="file"
                      accept=".json"
                      onChange={importMetricsFromFile}
                      disabled={isSyncing}
                ...
                {/* Instruções */}
                <div className="bg-factory-700/30 rounded-lg p-4 space-y-2">
                  <h5 className="text-neon-blue font-medium">📋 Como usar:</h5>
                  <div className="text-sm text-tire-300 space-y-1">
                    <p><strong>1.</strong> No navegador atual: Clique em "Baixar Arquivo de Métricas"</p>
                    <p><strong>2.</strong> No novo navegador: Abra este sistema e clique em "Backup/Restore"</p>
                    <p><strong>3.</strong> Selecione o arquivo JSON baixado</p>
                    <p><strong>4.</strong> Aguarde a mensagem de confirmação</p>
                    <p className="text-neon-green"><strong>✅ Pronto!</strong> Todos os dados estarão sincronizados</p>
                  </div>
                </div>
              </div>
