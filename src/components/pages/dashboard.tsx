...
const MainDashboard = ({ isLoading = false }: { isLoading?: boolean }) => {
  ...
  // Estado para sincronização com TireCostManager
  ...
  // Chaves unificadas para sincronização
  const TIRE_COST_STORAGE_KEY = "dashboard_averageCostPerTire";
  const PROFIT_PERCENTAGE_STORAGE_KEY = "dashboard_averageProfitPercentage";

  // Função para carregar valor sincronizado do localStorage
  const loadSynchronizedCostPerTire = () => {
    try {
      const saved = localStorage.getItem(TIRE_COST_STORAGE_KEY);
      if (saved) {
        const data = JSON.parse(saved);
        if (data.value && data.value > 0) {
          console.log(
            `🔄 [Dashboard] CARREGANDO valor sincronizado do TireCostManager: R$ ${data.value.toFixed(2)}`,
            {
              source: data.source,
          ...
          setSynchronizedCostPerTire(data.value);
          return data.value;
        }
      }
    } catch (error) {
      console.error(
        "❌ [Dashboard] Erro ao carregar valor sincronizado:",
        error,
      );
    }
    return 0;
  ...
  // Função para atualizar o custo médio por pneu das métricas principais
  const updateMainMetricsCost = () => {
    const newCost = calculateWeightedAverageCost();
    const currentMainMetrics = dataManager.loadData(
      "tire-factory-main-metrics",
      getDefaultMainMetrics(),
    );

    if (Math.abs(currentMainMetrics.averageCostPerTire - newCost) > 0.01) {
      console.log(
        `🔄 [Dashboard] ATUALIZANDO MÉTRICAS PRINCIPAIS: R$ ${currentMainMetrics.averageCostPerTire.toFixed(2)} → R$ ${newCost.toFixed(2)}`,
      );

      const updatedMetrics = {
        ...currentMainMetrics,
        averageCostPerTire: newCost,
        lastUpdated: new Date().toISOString(),
      };

      dataManager.saveData("tire-factory-main-metrics", updatedMetrics);
      setMainMetrics(updatedMetrics);
    }
  };

  // Efeito para sincronizar métricas principais quando o valor do TireCostManager mudar
  useEffect(() => {
    if (synchronizedCostPerTire > 0) {
      ...
      if (Math.abs(currentMainMetrics.averageCostPerTire - synchronizedCostPerTire) > 0.01) {
        console.log(
          `🔗 [Dashboard] SINCRONIZANDO métricas principais com TireCostManager: R$ ${currentMainMetrics.averageCostPerTire.toFixed(2)} → R$ ${synchronizedCostPerTire.toFixed(2)}`,
        );

        const updatedMetrics = {
          ...currentMainMetrics,
          averageCostPerTire: synchronizedCostPerTire,
          lastUpdated: new Date().toISOString(),
        };
  ...
  // Garantir sincronização no carregamento inicial
  useEffect(() => {
    // Carregar valor sincronizado do TireCostManager
    const initialCost = loadSynchronizedCostPerTire();
    loadSynchronizedProfitPercentage();

    // Forçar sincronização inicial das métricas principais
    if (initialCost > 0) {
      const currentMainMetrics = dataManager.loadData(
        "tire-factory-main-metrics",
        getDefaultMainMetrics(),
      ...
      if (Math.abs(currentMainMetrics.averageCostPerTire - initialCost) > 0.01) {
        console.log(
          `🔗 [Dashboard] SINCRONIZAÇÃO INICIAL: Atualizando métricas principais com valor do TireCostManager: R$ ${initialCost.toFixed(2)}`,
        );

        const updatedMetrics = {
          ...currentMainMetrics,
          averageCostPerTire: initialCost,
          lastUpdated: new Date().toISOString(),
        };
    ...
    // Verificar mudanças no localStorage do TireCostManager periodicamente
    const checkTireCostUpdates = () => {
      try {
        const saved = localStorage.getItem(TIRE_COST_STORAGE_KEY);
        if (saved) {
          const data = JSON.parse(saved);
          if (
            data.value &&
            data.value > 0 &&
            Math.abs(data.value - synchronizedCostPerTire) > 0.01
          ) {
            console.log(
              `🔄 [Dashboard] DETECTADA MUDANÇA no TireCostManager: R$ ${synchronizedCostPerTire.toFixed(2)} → R$ ${data.value.toFixed(2)}`,
            );
            setSynchronizedCostPerTire(data.value);
          }
        }
        ...
        console.error(
          "❌ [Dashboard] Erro ao verificar atualizações:",
          error,
        );
      }
    };

    // Intervalo para verificar atualizações (opcional, dependendo da necessidade)
    const intervalId = setInterval(checkTireCostUpdates, 5000); // A cada 5 segundos

    // Limpar intervalo ao desmontar o componente
    return () => clearInterval(intervalId);
  }, []);

  // Calcular o custo médio ponderado por tipo de pneu
  const calculateWeightedAverageCost = () => {
    // PRIORIDADE 1: Usar valor sincronizado do TireCostManager se disponível
    if (synchronizedCostPerTire > 0) {
      console.log(
        `🔗 [Dashboard] USANDO valor sincronizado do TireCostManager: R$ ${synchronizedCostPerTire.toFixed(2)}`,
      );
      return synchronizedCostPerTire;
    }

    // PRIORIDADE 2: Calcular baseado nos produtos se não houver valor sincronizado
    let totalCost = 0;
    let totalQuantity = 0;

    console.log("🧮 [Dashboard] Calculando custo médio ponderado (fallback)...");

    stockItems
      .filter((item) => item.item_type === "product")
      .forEach((product) => {
        const cost = getProductCost(product.item_name);
        if (cost > 0 && product.quantity > 0) {
          const productTotalCost = cost * product.quantity;
          totalCost += productTotalCost;
          totalQuantity += product.quantity;

          console.log(
            `📊 [Dashboard] Produto ${product.item_name}: ${product.quantity} × R$ ${cost.toFixed(2)} = R$ ${productTotalCost.toFixed(2)}`,
          );
        }
      });
    ...
    console.log(
      `💰 [Dashboard] Custo médio ponderado calculado (fallback): R$ ${averageCost.toFixed(2)}`,
      {
