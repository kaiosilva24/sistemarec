...
const ResaleProductsStock = ({ isLoading = false }: ResaleProductsStockProps) => {
  ...
  React.useEffect(() => {
    if (!resaleProductsLoading && !stockLoading && totalStockValue >= 0) {
      try {
        console.log(`💰 [ResaleProductsStock] Sincronizando valor total: R$ ${totalStockValue.toFixed(2)}`);
        ...
        console.log(`✅ [ResaleProductsStock] Valor total sincronizado: R$ ${totalStockValue.toFixed(2)}`);
        
      } catch (error) {
        console.error('❌ [ResaleProductsStock] Erro ao sincronizar valor total:', error);
      }
    }
  }, [totalStockValue, resaleProductsLoading, stockLoading]);
